...
function sendState(initial=false,action=null) {
  ...
  physics.nodes.forEach((n,i)=>{const k=i*3;positions[k]=n.x;positions[k+1]=n.y;positions[k+2]=n.z;pins[i]=Number(n.pinned);});
  const scales=physics.scales.slice(),linkProgress=physics.links.slice();
  const groupParents=[...physics.groupParents].map(([id,parent])=>[id,parent.position.x,parent.position.y,parent.position.z]);
  postMessage({type:'state',initial,action,cycle,positions,pins,metrics,scales,linkProgress,groupParents,entrance:physics.entrance,
...
function tick() {
  ...
  const started=performance.now(),running=settings.visible&&(settings.running||!!physics.drag||!!physics.groupDrag);
  if(running) {
    const wasDragging=!!physics.drag;
    accumulator+=Math.min(.05,(started-last)/1000);
    let count=0;
    while(accumulator>=1/60&&count<3) {
      const t=performance.now(),groupEnded=physics.step({...settings,drift:settings.drift&&settings.running});
      totalMs+=performance.now()-t;totalSteps++;stepCount++;count++;accumulator-=1/60;
      if(groupEnded)sendState(false,'groupEnded');
    }
...
self.onmessage=async({data})=>{
  try {
    ...
    if(!physics)return;
    if(data.type==='pointer'){physics.setPointer(data.pointer);return;}
    ...
    // Pointer messages update the target without resetting the simulation's
    // timer or publishing duplicate unchanged snapshots between fixed steps.
    if(data.type==='moveDrag'){physics.moveDrag(data.target);return;}
    if(data.type==='endDrag')physics.endDrag(data.pin);
    if(data.type==='startGroupDrag')physics.startGroupDrag(data.indices,data.centre,data.groupId,data.collisionProxies);
    if(data.type==='moveGroupDrag'){physics.moveGroupDrag(data.target);return;}
    if(data.type==='endGroupDrag'){physics.endGroupDrag();return;}
